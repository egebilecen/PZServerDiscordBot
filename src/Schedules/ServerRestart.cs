...
public static partial class Schedules
{
    public static void ServerRestart(List<object> args)
    {
        ...
        {
            Logger.WriteLog(string.Format("[{0}][Server Restart Schedule] serverRestartAnnouncer is null.", DateTime.UtcNow.ToLocalTime()));
            return;
        }
        ...
        Logger.WriteLog(string.Format("[{0}][Server Restart Schedule] Restarting server. (Is server running: {1})", DateTime.UtcNow.ToLocalTime(), isServerRunning.ToString()));
